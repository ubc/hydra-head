...
describe Hydra::PolicyAwareAccessControlsEnforcement do
  ...
  describe "policies_with_access" do
    context "Authenticated user" do
      ...
      it "returns the policies that provide discover permissions" do
        @policies_with_access.map(&:id).each do |p|
          expect(subject.policies_with_access).to include(p)
        end
        expect(subject.policies_with_access).not_to include("test-policy_no_access")
      end

      it "allows you to configure which model to use for policies" do
  ...
  describe "apply_policy_role_permissions" do
    it "escapes slashes in the group names" do
    ...
    it "escapes spaces in the group names" do
