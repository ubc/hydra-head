...
describe Hydra::AdminPolicy do
  ...
  describe "to_solr" do
    subject { described_class.new(title: ["Foobar"]).to_solr }
    it "has title_ssim" do
  ...
  describe "updating default permissions" do
    ...
    it "does not replace existing users" do
      subject.default_permissions.build(name: 'user1', access: 'discover', type: 'person')
    ...
    it "assigns user permissions when :type == 'person'" do
      subject.default_permissions.build(name: 'user1', access: 'discover', type: 'person')
      expect(subject.default_permissions.map(&:to_hash)).to eq [{ type: 'person', access: 'discover', name: 'user1' }]
    end
    it "raises an ArgumentError when the :type hashkey is invalid" do
  ...
  describe "Inheritable rights" do
    ...
    describe "indexing" do
      ...
      it "does not affect normal solr permissions fields" do
        expect(subject).not_to have_key Hydra.config.permissions.discover.group
        expect(subject).not_to have_key Hydra.config.permissions.discover.individual
        expect(subject).not_to have_key Hydra.config.permissions.read.group
        expect(subject).not_to have_key Hydra.config.permissions.read.individual
        expect(subject).not_to have_key Hydra.config.permissions.edit.group
        expect(subject).not_to have_key Hydra.config.permissions.edit.individual
        expect(subject).not_to have_key Hydra.config.permissions.embargo.release_date
      end

      it "provides prefixed/inherited solr permissions fields" do
