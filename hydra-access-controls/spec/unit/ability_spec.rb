...
describe Ability do
  ...
  describe "class methods" do
    ...
    its(:read_group_field) { is_expected.to eq('read_access_group_ssim') }
    its(:read_user_field) { is_expected.to eq('read_access_person_ssim') }
    its(:edit_group_field) { is_expected.to eq('edit_access_group_ssim') }
    its(:edit_user_field) { is_expected.to eq('edit_access_person_ssim') }
    its(:discover_group_field) { is_expected.to eq('discover_access_group_ssim') }
    its(:discover_user_field) { is_expected.to eq('discover_access_person_ssim') }
  end
  ...
  context "for a not-signed in user" do
    ...
    it "calls custom_permissions" do
      expect_any_instance_of(described_class).to receive(:custom_permissions)
      subject.can?(:delete, 7)
    end
    it { is_expected.not_to be_able_to(:create, ActiveFedora::Base) }
  end
  ...
  describe "Given an asset that has been made publicly discoverable" do
    ...
    context "Then a not-signed-in user" do
      ...
      it { is_expected.not_to be_able_to(:edit, asset) }
  ...
  describe "Given an asset with no custom access set" do
    ...
    context "Then a registered user" do
      subject { described_class.new(FactoryGirl.build(:registered_user)) }
      it { is_expected.not_to be_able_to(:discover, asset) }
      it { is_expected.not_to be_able_to(:read, asset) }
      it { is_expected.not_to be_able_to(:edit, asset) }
      it { is_expected.not_to be_able_to(:update, asset) }
    ...
    context "Then the Creator" do
      subject { described_class.new(FactoryGirl.build(:joe_creator)) }
      it { is_expected.to     be_able_to(:discover, asset) }
      it { is_expected.to     be_able_to(:read, asset) }
      it { is_expected.to     be_able_to(:edit, asset) }
      ...
      it { is_expected.to     be_able_to(:destroy, solr_doc) }
      it { is_expected.not_to be_able_to(:admin, asset) }
    end
  end
  ...
  describe "custom method" do
    ...
    it { is_expected.to be_able_to(:accept, ActiveFedora::Base) }
  end
  ...
  describe "calling ability on two separate objects" do
    ...
    it "is readable in the first instance and not in the second instance" do
      ...
      expect(subject).not_to be_able_to(:edit, asset2)
    end
  end
